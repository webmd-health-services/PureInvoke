...
Describe 'Invoke-AdvApiQueryServiceConfig2' {
    ...
    Context 'service that is delayed auto-start, has a description, failure actions, required privileges, SID type' {
        BeforeAll {
            $script:svcHandle = Invoke-AdvApiOpenService -SCManagerHandle $script:scmHandle -ServiceName 'BITS'
        }
        ...
        It 'reads delayed auto start' {
            ...
            $info.DelayedAutoStart | Should -Be $true
        }
    ...
    Context 'service that runs failure actions on non-crash failures' {
        BeforeAll {
            $script:svcHandle = Invoke-AdvApiOpenService -SCManagerHandle $script:scmHandle -ServiceName 'EventLog'
        }
        ...
        It 'reads failure action flags' {
            ...
            ThenError -IsEmpty
            $info | Should -Not -BeNullOrEmpty
            $info.FailureActionsOnNonCrashFailures | Should -Be $true
        }
    }
    ...
    Context 'service that configures preshutdown timeout' {
        BeforeAll {
            $script:svcHandle =
                Invoke-AdvApiOpenService -SCManagerHandle $script:scmHandle -ServiceName 'TrustedInstaller'
        }
    ...
    Context 'service that configures multiple triggers' {
        BeforeAll {
            $script:svcHandle =
                Invoke-AdvApiOpenService -SCManagerHandle $script:scmHandle -ServiceName 'ClipSVC'
        }
    ...
    Context 'service that configures trigger with multiple data items' {
        BeforeAll {
            ...
            $script:svcHandle =
                Invoke-AdvApiOpenService -SCManagerHandle $script:scmHandle -ServiceName $svcName
        }
    ...
    Context 'protected service' {
        BeforeAll {
            $script:svcHandle =
                Invoke-AdvApiOpenService -SCManagerHandle $script:scmHandle -ServiceName 'AppIDSvc'
        }
    ...
    It 'queries <_> service' -ForEach $svcNames {
        $svcHandle =
            Invoke-AdvApiOpenService -SCManagerHandle $script:scmHandle -ServiceName $_
