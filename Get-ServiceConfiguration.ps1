...
.DESCRIPTION
The `Get-Service-Configuration` calls the Windows API's `QueryServiceConfig2` function. This is hard to do from an
interactive session becuase it is a five step process: open the Service Control Manager, open the service, query the
service, close the service, then close the service control manager. Use this script to inspect the objects and values
returned by `QueryServiceConfig2`. Pass the name of the service (wildards accepted) to the `Name` parameter and the
information to return to the `InfoLevel` parameter.

.EXAMPLE
.\Get-ServiceConfiguration.ps1 -Name 'W32Time' -InfoLevel Triggers

Demonstrates how to use this script to call the `QueryServiceConfig2` Windows API function. In this example, the
W32Time service's triggers configuration is returned.
#>
[CmdletBinding()]
param(
